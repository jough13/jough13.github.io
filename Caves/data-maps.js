...
window.TILE_DATA = {
    ...
    '🏛️c': {
        ...
        name: 'The Lost City of the Sands',
        flavor: "Massive sandstone pillars rise from the dunes. The gates to the Lost City stand open.",
        getCastleId: (x, y) => `lost_city_${x}_${y}`,
        onInteract: (state, x, y) => {
            // Re-using the landmark_castle logic natively!
            state.mapMode = 'castle';
            state.currentCastleId = `lost_city_${x}_${y}`;
            state.overworldExit = { x: state.player.x, y: state.player.y };
            
            // Force the generator to use our new layout
            chunkManager.generateCastle(state.currentCastleId, 'LOST_CITY');
            const spawn = chunkManager.castleSpawnPoints[state.currentCastleId];
            state.player.x = spawn.x;
            state.player.y = spawn.y;
            
            // Populate with Undead
            const baseEnemies = chunkManager.castleEnemies[state.currentCastleId] || [];
            state.instancedEnemies = JSON.parse(JSON.stringify(baseEnemies));
            state.friendlyNpcs = []; // No friends here!
            
            logMessage("{gold:You step into the ancient, cursed streets of the Lost City...}");
        ...
        onInteract: (state, x, y) => {
            ...
            setTimeout(() => {
                ...
                if (starBtn) starBtn.onclick = (e) => {
                    ...
                    if (inv.length >= (typeof getInventoryCap === 'function' ? getInventoryCap(p) : 9)) {
        ...
        onInteract: (state, x, y) => {
            ...
            if (player.shadowQuestStage === 0) {
                ...
                if (amuletIdx > -1) {
                    ...
                    if (inv.length < (typeof getInventoryCap === 'function' ? getInventoryCap(player) : 9)) {
