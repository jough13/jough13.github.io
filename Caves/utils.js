...
function initEventListeners() {
    ...
    EventBus.on('levelUp', (data) => {
        ...
        EventBus.on('restAttempted', (data) => {
        switch (data.status) {
            case 'success':
                if (data.stamina > 0) triggerStatFlash(statDisplays.stamina, true);
                if (data.hp > 0) triggerStatFlash(statDisplays.health, true);
                
                if (data.isSafeZone) {
                    logMessage(`You rest comfortably in the haven. (+${data.hp + data.stamina} HP/Stamina)`);
                } else {
                    logMessage(`You rest for a moment. Recovered ${data.stamina} stamina and ${data.hp} health.`);
                }
                break;
            case 'wellRested':
                logMessage(`{gold:You feel Well Rested! (${data.bonus} for ${data.duration} turns)}`);
                triggerStatAnimation(statDisplays.strength, 'stat-pulse-green');
                renderEquipment();
                break;
            case 'failed':
                logMessage(`You are too weak to rest effectively.`);
                break;
            case 'alreadyRested':
...
function restPlayer() {
    ...
    if (player.hunger <= 0 || player.thirst <= 0) {
        EventBus.emit('restAttempted', { status: 'failed', reason: 'Too weak from hunger or thirst.' });
        endPlayerTurn();
        return;
    }
    ...
    if (player.stamina < player.maxStamina) {
        staminaRecovered = Math.min(player.maxStamina - player.stamina, restAmount);
        player.stamina += staminaRecovered;
    }

    if (player.health < player.maxHealth) {
        healthRecovered = Math.min(player.maxHealth - player.health, restAmount);
        player.health += healthRecovered;
    }
    
    const rested = staminaRecovered > 0 || healthRecovered > 0;

    if (inSafeZone && !rested) {
        if (player.strengthBonusTurns < 10) {
            player.strengthBonus = 2;
            player.strengthBonusTurns = 50;
            EventBus.emit('restAttempted', { status: 'wellRested', bonus: '+2 Strength', duration: 50 });
        } else {
            EventBus.emit('restAttempted', { status: 'alreadyRested' });
        }
    } else if (rested) {
        EventBus.emit('restAttempted', { status: 'success', hp: healthRecovered, stamina: staminaRecovered, isSafeZone: inSafeZone });
    } else {
        EventBus.emit('restAttempted', { status: 'full' });
    }

    endPlayerTurn(); // This is core game loop logic, so it stays.
}
...
if (enemy) {
    ...
    return;
            ...
            gameState.lastMoveStart = Date.now(); 
