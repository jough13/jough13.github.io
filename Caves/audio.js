...
const AudioSystem = {
    ...
    // 🚨 MEMORY LEAK FIX & GC WIN: Hardware-Accurate Silent Timer
    // `setTimeout` throttles when the browser tab is hidden, breaking reverb tails and 
    // causing massive lag spikes when tabbing back in. By spawning a silent oscillator,
    // we harness the Web Audio API's native audio thread which NEVER stops tracking time!
    _scheduleEffectCleanup: function(ctx, pannerNode, delayTimeSec) {
        if (!pannerNode) return;
        
        const timerOsc = ctx.createOscillator();
        const silentGain = ctx.createGain();
        silentGain.gain.value = 0; // Absolutely silent
        
        timerOsc.connect(silentGain);
        silentGain.connect(ctx.destination);
        
        timerOsc.onended = () => {
            try {
                // Wipe out the delay chain to free memory
        ...
        timerOsc.start(ctx.currentTime);
        timerOsc.stop(ctx.currentTime + delayTimeSec);
    },
    ...
    playNoise: function(duration, vol = 0.1, filterFreq = 1000, x = null, isUI = false) {
        ...
        // Drop the source node instantly on completion
        src.onended = () => {
            try { 
        ...
        // Clean up the reverb tail with the silent hardware timer
        const tail = this._getTailTime(acoustics);
        this._scheduleEffectCleanup(ctx, panner, actualDuration + tail + 0.1);
    },
    ...
    playMelody: function(notes, type = 'sine', speed = 0.15, vol = 0.1, isUI = false) {
        ...
        const tail = this._getTailTime(acoustics);
        this._scheduleEffectCleanup(ctx, panner, totalDuration + tail + 0.1);
    },
