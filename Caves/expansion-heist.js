...
window.ExpansionManager.register({
    ...
    data: {
        ...
        items: {
            ...
            '🧪s': {
                ...
                effect: (state) => {
                    ...
                    logMessage("{gray:You drink the draught and melt seamlessly into the shadows...}");
                    if (typeof AudioSystem !== 'undefined') AudioSystem.playMagic();
                    if (typeof ParticleSystem !== 'undefined') ParticleSystem.createExplosion(state.player.x, state.player.y, '#374151', 20);
                    return true;
                }
            }
        },
        ...
        shops: {
            ...
            // Added to the Black Market natively
            black_market: [
                { name: 'Lockpick', price: 100, stock: 10 },
                { name: 'Shadow Draught', price: 500, stock: 2 },
                { name: 'Blood Ruby', price: 10, stock: 0 } // They will buy it from you!
            ]
        },
        alchemyRecipes: {
            "Shadow Draught": { materials: { "Void Dust": 2, "Bat Wing": 1, "Empty Bottle": 1 }, xp: 60, level: 4, yield: 1 }
        }
    },
    ...
    init: function() {
        ...
        window.StealthManager = {
            ...
            updateUI: function() {
                ...
                // Safe DOM check for early loading
                if (!bar || !valTxt || !statTxt || !container) return;
            ...
            processTurn: function(oldX, oldY) {
                const p = gameState.player;
                if (!p.isCrouching) return;
                if (p.health <= 0 || gameState.isDead) return; // Prevent dead players accumulating suspicion

                // 1. Maintain Stealth Buff to trick the Enemy AI Loop!
                // 🚨 BUG FIX WIN: Use Math.max to prevent overriding a 15-turn Shadow Draught!
        ...
        // Because expansions load asynchronously, we verify existence first!
        
        const npcTiles = ['¥', '§', 'G', 'H'];
        
        npcTiles.forEach(tChar => {
            if (window.TILE_DATA && window.TILE_DATA[tChar]) {
                const originalInteract = window.TILE_DATA[tChar].onInteract;
                
                window.TILE_DATA[tChar].onInteract = (state, x, y) => {
                    // Call the original interact (from core or Syndicate) to build the modal
                    const res = originalInteract ? originalInteract(state, x, y) : false;
                    
                    // 🚨 BUG FIX: If the parent returned false (not crouching in Syndicate), pass it downward!
                    // This allows the engine to fall through to the normal shop/dialogue menu cleanly.
                    if (res === false) return false;
                    
                    // Set a timeout to safely inject our button into the rendered modal
                    setTimeout(() => {
                        // Hook into Syndicate's mug button if it exists, otherwise just inject it generally
                        const mugBtn = document.getElementById('npcMugBtn');
                        const loreContent = document.getElementById('loreContent');
                        
                        if (loreContent && !document.getElementById('npcPickpocketBtn')) {
                            ...
                            if (mugBtn) mugBtn.parentNode.insertBefore(ppBtn, mugBtn);
                            else loreContent.appendChild(ppBtn);
                            ...
                            ppBtn.onclick = () => {
                                ...
                                if (Math.random() < chance) {
                                    ...
                                    // --- LORE WIN: NPC-Specific Pickpocket Loot Tables! ---
                                    let bonusLoot = null;
                                    const roll = Math.random();
                                    if (tChar === 'G' && roll < 0.20) bonusLoot = { id: '♥', name: 'Healing Potion' };
                                    else if (tChar === 'H' && roll < 0.30) bonusLoot = { id: '🧪a', name: 'Antidote' };
                                    else if (tChar === '¥' && roll < 0.15) bonusLoot = { id: '🗺️', name: 'Tattered Map' };
                                    else if (tChar === '§' && roll < 0.10) bonusLoot = { id: '💰b', name: 'Blood Ruby' };
                                    
                                    if (bonusLoot) {
                                        const template = typeof window.ITEM_DATA !== 'undefined' ? window.ITEM_DATA[bonusLoot.id] : null;
                                        ...
                                        if (template && p.inventory.length < invCap) {
                                            ...
                                            newItem.templateId = bonusLoot.id; newItem.quantity = 1; newItem.isEquipped = false;
                                            p.inventory.push(newItem);
                                            logMessage(`{purple:You also snagged a ${bonusLoot.name}!}`);
                                        }
                                    }
                                ...
                                if (typeof triggerDebouncedSave === 'function') {
                                    triggerDebouncedSave({ coins: p.coins, bounty: p.bounty || 0, inventory: typeof getSanitizedInventory === 'function' ? getSanitizedInventory() : p.inventory });
                                }
                            };
                        }
                    }, 20); 
                    
                    return res; // Pass up the chain
                };
            }
        });
