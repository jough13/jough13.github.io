    ...
    // --- STANDARD ITEM PICKUP ---
        ...
        if (itemData.type === 'random_lore') {
            ...
            // HANDLE ALL PICKUPABLE ITEMS
                ...
                // The "Infinite Value" Equipment Stacking Exploit Fix
                // Use the global stackable helper if available to ensure absolute consistency across the game, 
                // adding quest/lore items since they are safe to stack but not stashable.
                const isStackable = typeof window.isStackableItem === 'function' 
                    ? (window.isStackableItem(itemData.type) || itemData.type === 'quest' || itemData.type === 'lore')
                    : ['junk', 'consumable', 'trade', 'ingredient', 'ammo', 'quest', 'lore'].includes(itemData.type);
                
                // Ensure we only find an existing item IF the item type is actually stackable!
                // This prevents finding a "Rusty Sword" and trying to stack it, forcing it to take a new slot.
                const existingItem = isStackable ? gameState.player.inventory.find(item => item.name === itemData.name && !item.isEquipped) : null;
                const invCap = typeof getInventoryCap === 'function' ? getInventoryCap(gameState.player) : 9;
