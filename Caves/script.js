...
async function enterGame(playerData) {
    ...
    if (playerData.inventory) {
        playerData.inventory = typeof window.rehydrateItemArray === 'function' ? window.rehydrateItemArray(playerData.inventory) : rehydratePlayerState(playerData);
    }

    if (playerData.bank) {
        playerData.bank = typeof window.rehydrateItemArray === 'function' ? window.rehydrateItemArray(playerData.bank) : (playerData.bank || []);
    }

    if (playerData.spireBackupInv) {
        playerData.spireBackupInv = typeof window.rehydrateItemArray === 'function' ? window.rehydrateItemArray(playerData.spireBackupInv) : (playerData.spireBackupInv || []);
    }
