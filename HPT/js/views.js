        ...
        <div className="p-4 sm:p-6 space-y-6">
            ...
            {isNewUser && (
                <div className="relative bg-white dark:bg-slate-800 rounded-xl p-6 shadow-lg border border-slate-200 dark:border-slate-700 border-l-8 border-l-sky-500 animate-fade-in group">
                    ...
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2 pr-8">
                        Welcome to the Health Physics Toolbox
                    </h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {favoriteNuclides.map(n => (
                            <NuclideCard
                                ...
                                nuclide={n}
                                isCompact={true}
                                onNuclideClick={(n) => handleNuclideSelection(n, true)} // Pass true to skip adding this to search history again
                            />
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        {searchHistoryNuclides.map(n => (
                            <NuclideCard
                                ...
                                nuclide={n}
                                isCompact={true}
                                onNuclideClick={(n) => handleNuclideSelection(n, true)} // Pass true here too
                            />
                        ))}
                    </div>
                </div>
        ...
        <>
            <div className="flex justify-between items-end mb-2">
                ...
                <div className="flex bg-slate-200 dark:bg-slate-700 rounded-lg p-1">
                    <button
                        ...
                        className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${viewMode === 'edit' ? 'bg-white dark:bg-slate-600 shadow text-sky-600' : 'text-slate-500'}`}
                    >
                        Edit
                    </button>
                    <button
                        onClick={() => setViewMode('preview')}
                        className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${viewMode === 'preview' ? 'bg-white dark:bg-slate-600 shadow text-sky-600' : 'text-slate-500'}`}
                    >
                        Preview
                    </button>
                </div>
            ...
            <div className="bg-slate-100 dark:bg-slate-700/50 rounded-t-md border-b border-slate-200 dark:border-slate-700 p-2 flex flex-wrap items-center gap-2">
                <button
                    ...
                    className="px-3 py-1.5 text-xs bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded hover:bg-slate-50 dark:hover:bg-slate-500 transition font-medium flex items-center gap-1"
                >
                    <Icon path={ICONS.stopwatch} className="w-3 h-3" />
                    Timestamp
                </button>
                <button
                ...
                <button
                    ...
                    className="px-3 py-1.5 text-xs bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded hover:bg-slate-50 dark:hover:bg-slate-500 transition font-medium flex items-center gap-1"
                >
                    <Icon path={ICONS.database} className="w-3 h-3" />
                    Save .txt
                </button>
                <div className="flex-grow"></div>
                <button
                    onClick={() => setIsClearModalOpen(true)}
                    className="text-xs text-red-500 hover:text-red-700 font-bold flex items-center gap-1 px-2"
                >
            ...
            items: [
                {
                    ...
                    description: 'Standards for Protection Against Radiation (US NRC).'
                },
                {
                    title: '49 CFR Part 173',
                    url: 'https://www.ecfr.gov/current/title-49/subtitle-B/chapter-I/subchapter-C/part-173',
                    description: 'DOT Regulations for shipping hazardous materials (Class 7).'
                },
                {
                    title: 'NUREG-1556 Series',
                    url: 'https://www.nrc.gov/reading-rm/doc-collections/nuregs/staff/sr1556/index.html',
                    description: 'Consolidated Guidance About Materials Licenses.'
                },
                {
                    title: 'MARSSIM',
                    url: 'https://www.nrc.gov/reading-rm/doc-collections/nuregs/staff/sr1575/index.html',
                    description: 'Multi-Agency Radiation Survey and Site Investigation Manual.'
                }
            ]
        },
        {
            ...
            items: [
                {
                    ...
                    description: 'Authoritative nuclear structure and decay data.'
                },
                {
                    title: 'NIST XCOM',
                    url: 'https://physics.nist.gov/PhysRefData/Xcom/html/xcom1.html',
                    description: 'Photon Cross Sections Database (Attenuation Coefficients).'
                },
                {
                    title: 'Rad Pro Calculator',
                    url: 'http://www.radprocalculator.com/',
                    description: 'Comprehensive online health physics calculations.'
                },
                {
                    title: 'EPA Radiation',
                    url: 'https://www.epa.gov/radiation',
                    description: 'US EPA radiation protection information and resources.'
                },
                {
                    title: 'Plexus-NSD',
                    url: 'http://www.iem-inc.com/information/tools/gamma-ray-dose-constants',
                    description: 'Source for Specific Gamma Ray Constants.'
                },
                {
                    title: 'REMM',
                    url: 'https://remm.hhs.gov/',
                    description: 'Radiation Emergency Medical Management (HHS).'
                }
            ]
        },
        {
            ...
            items: [
                {
                    title: 'Health Physics Society (HPS)',
                    url: 'https://hps.org/',
                    description: 'Scientific organization of radiation safety professionals.'
                },
                {
                    title: 'AAPM',
                    url: 'https://www.aapm.org/',
                    description: 'American Association of Physicists in Medicine.'
                },
                {
                    title: 'IAEA',
                ...
                <div className="space-y-6">
                    {referenceLinks.map(section => (
                        <div key={section.category}>
                            <h3 className="text-lg font-semibold text-sky-600 dark:text-sky-400 border-b border-slate-300 dark:border-slate-600 pb-2 mb-3">
                                {section.category}
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {section.items.map(item => (
                                    <a
                                        ...
                                        key={item.title}
                                        className="block p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-sky-400 dark:hover:border-sky-500 hover:shadow-md transition-all group"
                                    >
...
const FilterSidebar = ({
    ...
    const {
        ...
        setSelectedEmissionType,
        allEmissionTypes,
        selectedCommonality,
        setSelectedCommonality,
        allCommonalityCategories,
        minAlphaEnergy,
        setMinAlphaEnergy,
        maxAlphaEnergy,
        setMaxAlphaEnergy,
    ...
    return (
        <>
            <div
                ...
                aria-hidden="true"
            ></div>
            <aside className={`fixed top-0 left-0 w-80 h-full bg-slate-50 dark:bg-slate-900 z-50 transform transition-transform duration-300 ease-in-out lg:relative lg:w-auto lg:h-auto lg:transform-none lg:col-span-1 ${isFiltersVisible ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-4 space-y-4 h-full overflow-y-auto">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="mt-1 w-full p-2 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded-md"
                        >
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Emission Type</label>
                        <select
                            value={selectedEmissionType}
                            onChange={e => setSelectedEmissionType(e.target.value)}
                            className="mt-1 w-full p-2 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded-md"
                        >
                            {allEmissionTypes.map(c =>
                                <option key={c} value={c}>{c}</option>
                            )}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Commonality</label>
                        <select
                            value={selectedCommonality}
                            onChange={e => setSelectedCommonality(e.target.value)}
                            className="mt-1 w-full p-2 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded-md"
                        >
                            {allCommonalityCategories.map(c =>
                                <option key={c} value={c}>{c}</option>
                            )}
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 mt-2">
                            <input
                                ...
                                placeholder="Min"
                                value={minHalfLife}
                                onChange={e => setMinHalfLife(e.target.value)}
                                className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600"
                            />
                            <input
                                type="number"
                                placeholder="Max"
                        ...
                        <select
                            value={halfLifeFilterUnit}
                            onChange={e => setHalfLifeFilterUnit(e.target.value)}
                            className="mt-1 w-full p-2 bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-600 rounded-md"
                        >
                    ...
                    <div>
                        ...
                        <div className="space-y-2 mt-2">
                            ...
                            <div className="flex gap-2">
                                <input type="number" placeholder="Min" value={minAlphaEnergy} onChange={e => setMinAlphaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                                <input type="number" placeholder="Max" value={maxAlphaEnergy} onChange={e => setMaxAlphaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                            </div>
                            <label className="text-sm">Beta</label>
                            <div className="flex gap-2">
                                <input type="number" placeholder="Min" value={minBetaEnergy} onChange={e => setMinBetaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                                <input type="number" placeholder="Max" value={maxBetaEnergy} onChange={e => setMaxBetaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                            </div>
                            <label className="text-sm">Gamma</label>
                            <div className="flex gap-2">
                                <input type="number" placeholder="Min" value={minGammaEnergy} onChange={e => setMinGammaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                                <input type="number" placeholder="Max" value={maxGammaEnergy} onChange={e => setMaxGammaEnergy(e.target.value)} className="w-full p-1 rounded-md text-sm bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" />
                            </div>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="relative">
            <div className="px-4 pt-4 lg:hidden">
                ...
                >
                    <Icon path={ICONS.filter} className="w-5 h-5" />
                    Filters & Sorting
                </button>
            </div>
...
const UnitConverter = () => {
    ...
    React.useEffect(() => {
        ...
        if (category === 'Activity') {
            setFromUnit('Ci');
            setToUnit('GBq');
        } else if (category === 'Dose Equivalent') {
            setFromUnit('mrem');
            setToUnit('µSv');
        } else if (category === 'Contamination') {
            setFromUnit('dpm/100cm²');
            setToUnit('Bq/cm²');
        } else {
            setFromUnit(units[0]);
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-8">
                    {Object.keys(CATEGORIES).map(cat => (
                        <button
                            ...
                            className={`px-2 py-2 rounded-lg text-xs font-bold transition-all duration-200 border ${category === cat ? 'bg-sky-50 dark:bg-sky-900/30 border-sky-500 text-sky-700 dark:text-sky-300 shadow-sm ring-1 ring-sky-500' : 'bg-slate-50 dark:bg-slate-700/50 border-transparent text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                        >
                            {cat}
                        </button>
                    ))}
                        ...
                        <div className="px-4 pt-3 pb-1">
                            ...
                            <div className="flex items-center">
                                <input
                                    ...
                                    className="flex-1 w-0 bg-transparent text-2xl font-bold text-slate-800 dark:text-white focus:outline-none placeholder-slate-300 min-w-0"
                                    placeholder="0"
                                />
                                <div className="ml-2 pl-2 border-l border-slate-200 dark:border-slate-600 max-w-[40%] flex-shrink-0">
                                    <select
                                        value={fromUnit}
                                        onChange={e => setFromUnit(e.target.value)}
                                        className="w-full bg-transparent font-bold text-sm text-slate-600 dark:text-slate-300 focus:outline-none cursor-pointer hover:text-sky-600 truncate"
                                    >
                    ...
                    <div className="relative group bg-sky-50 dark:bg-sky-900/10 rounded-xl border border-sky-100 dark:border-sky-800/50 transition-all">
                        <div className="px-4 pt-3 pb-1">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={handleSave} className="text-sky-400 hover:text-sky-600 transition-colors">
                                    <Icon path={ICONS.notepad} className="w-4 h-4" />
                                </button>
                            </div>
                            <div className="flex items-center">
                                <div className="flex-1 w-0 text-2xl font-bold text-sky-700 dark:text-sky-400 truncate py-1 min-w-0">
                                    {result}
                                </div>
                                <div className="ml-2 pl-2 border-l border-sky-200 dark:border-sky-800 max-w-[40%] flex-shrink-0">
                                    <select
                                        value={toUnit}
...
const SettingsPanel = () => {
    ...
    // State to track if the browser allows PWA installation
    const [installPrompt, setInstallPrompt] = React.useState(window.deferredPwaPrompt);

    // Polling effect to catch the global variable if it populates after mount
    React.useEffect(() => {
        const interval = setInterval(() => {
            if (window.deferredPwaPrompt !== installPrompt) {
    ...
    const handleInstallClick = async () => {
        ...
        if (outcome === 'accepted') {
            ...
            addToast("Installing Health Physics Toolbox...");
        }
    };
        ...
        <>
            <div className="p-4 animate-fade-in">
                <div className="max-w-xl mx-auto bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
                    ...
                    {/* --- App Management (PWA) Section --- */}
                    <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700 space-y-4">
                        <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 mb-4">App Management</h3>

                        {installPrompt && (
                            <button
                                onClick={handleInstallClick}
                                className="w-full flex items-center justify-between p-3 bg-sky-600 hover:bg-sky-700 text-white rounded-lg font-bold transition-colors shadow-sm"
                            >
                                <span>Install to Device</span>
                                <Icon path={ICONS.download || "M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"} className="w-5 h-5" />
                            </button>
                        )}

                        {!installPrompt && (
                            <div className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-lg text-xs text-slate-500 dark:text-slate-400 text-center border border-slate-200 dark:border-slate-700">
                                <strong>Using an iPhone or iPad?</strong><br />
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-between p-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-lg font-bold border border-slate-300 dark:border-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
                        >
                            <span>Force Check for Updates</span>
                            <Icon path={ICONS.refresh || "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"} className="w-5 h-5" />
                        </button>
                    </div>
            ...
            <ConfirmationModal isOpen={isResetModalOpen} onClose={() => setIsResetModalOpen(false)} onConfirm={handleResetDefaults} title="Reset All Settings & Saved Data?">
                <p>Are you sure? This will clear <strong>all</strong> saved calculator inputs, favorites, search history, and default settings. The application will reload with its original defaults. This action cannot be undone.</p>
            </ConfirmationModal>
        </>
    );
};
...
const PopoutWindow = ({ children, title, onClose, width = 450, height = 750 }) => {
    ...
    React.useEffect(() => {
        ...
        // 2. Use '_blank' to completely bypass strict window naming rules
        newWindow.current = window.open('', '_blank', `width=${width},height=${height},resizable=yes`);
        
        // Safety check for aggressive pop-up blockers
        if (!newWindow.current) {
            alert(`Pop-up blocked! Please click the "Pop-up blocked" icon in your address bar and select "Always allow" to open the ${title}.`);
            onClose(); 
            return;
        }

        // 3. Write a complete, fresh HTML document to the new window.
        // This forces the browser/OS to parse the theme-color natively on load!
        const htmlContent = `
            <!DOCTYPE html>
        ...
        newWindow.current.document.write(htmlContent);
        newWindow.current.document.close();
        
        // 4. Copy over custom local styles (like your style.css)
        document.head.querySelectorAll('link[rel="stylesheet"], style').forEach(node => {
            // Don't copy meta tags here, we hardcoded the theme color perfectly above
            if (node.tagName !== 'META') {
                newWindow.current.document.head.appendChild(node.cloneNode(true));
            }
        });
        
        // 5. Target the div we explicitly created in the HTML string
        const popoutRoot = newWindow.current.document.getElementById('popout-root');
    ...
    }, []); // Empty dependency array ensures this strict build sequence only happens ONCE
    
    // Dynamic Theme Syncing (Allows dark mode toggling while popout is open)
    React.useEffect(() => {
        if (!newWindow.current || !newWindow.current.document) return;
        const isDark = document.documentElement.classList.contains('dark');
        const themeColor = isDark ? '#0f172a' : '#f8fafc';
        
        newWindow.current.document.documentElement.className = isDark ? 'dark' : '';
        const metaTheme = newWindow.current.document.querySelector('meta[name="theme-color"]');
        if (metaTheme) metaTheme.content = themeColor;
    }); // Runs on every render to keep synced with parent
                                    ...
                                    {comparisonList.map(nuclide => (
                                        <th key={nuclide.symbol} className="sticky top-0 z-20 bg-slate-100 dark:bg-slate-800 p-3 text-sm font-semibold border-b border-slate-200 dark:border-slate-600 min-w-[140px]">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <button onClick={() => onRemoveFromCompare(nuclide.symbol)} className="ml-2 text-slate-400 hover:text-red-500">
                                                    <Icon path={ICONS.clear} className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </th>
                                    ...
                                    return (
                                        <tr key={prop.label} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            ...
                                            {comparisonList.map((nuclide, idx) => {
                                                    ...
                                                    if (prop.type === 'numeric') {
                                                        ...
                                                        cellContent = (
                                                            <div className="relative h-full w-full flex items-center">
                                                                {barWidth > 0 && (
                                                                    <div
                                                                        className={`absolute bottom-0 left-0 h-1 transition-all duration-500 ${prop.highlightMode === 'highGood' ? (numVal === maxVal ? 'bg-green-500' : 'bg-green-200') : prop.highlightMode === 'lowGood' ? (numVal === minVal ? 'bg-green-500' : 'bg-slate-300') : 'bg-sky-400'}`}
                                                ...
                                                return (
                                                    <td key={nuclide.symbol} className={`${cellClass} p-3 text-sm`}>
                                                        {cellContent}
                                                    </td>
                                                );
                                            })}
