...
playerName = document.getElementById('playerNameInput').value || "Captain";
    ...
    const gameState = {
        ...
        // --- NEW: Save the World State (Chunks) ---
        // This persists the exact state of every sector you have visited.
    ...
    try {
        ...
        logMessage("Failed to save game. Storage quota might be exceeded.");
    }
}
...
function loadGame() {
    ...
    try {
        ...
        // --- Version Warning ---
        // If the save is from an older version, warn the user (or handle migration later)
        ...
        // --- NEW: Restore World State & Clear Cache ---
        if (savedState.loadedChunks) {
            chunkManager.loadedChunks = savedState.loadedChunks;
        } else {
            // Fallback for older saves that didn't have chunk data
            chunkManager.loadedChunks = {};
        }

        // Clear the optimization cache (from the performance fix) 
        // to ensure we don't display old data from before the load.
        if (chunkManager.lastChunkKey !== undefined) {
             chunkManager.lastChunkKey = null;
             chunkManager.lastChunkRef = null;
        }

        // --- Refresh game systems with loaded data ---
        currentSectorX = Math.floor(playerX / SECTOR_SIZE);
        currentSectorY = Math.floor(playerY / SECTOR_SIZE);
        currentSectorName = generateSectorName(currentSectorX, currentSectorY);
...
function renderSystemMap() {
    ...
    // --- 2. Clear the canvas (Modified to show CSS background) ---
    ctx.clearRect(0, 0, gameCanvas.width, gameCanvas.height); 
    
    // Optional: Add a semi-transparent overlay so the text pops against the stars
    ...
    // --- 4. Draw Sensor Pulse (New Feature) ---
    if (sensorPulseActive) {
        ctx.beginPath();
        ctx.arc(playerScreenX, playerScreenY - (TILE_SIZE/4), sensorPulseRadius, 0, 2 * Math.PI);
        ctx.strokeStyle = `rgba(0, 224, 224, ${1 - (sensorPulseRadius / MAX_PULSE_RADIUS)})`; // Fade out
        ctx.lineWidth = 2;
        ctx.stroke();
    }
...
function movePlayer(dx, dy) {
    ...
    if (dx !== 0 || dy !== 0) {
        ...
        playerFuel = parseFloat(playerFuel.toFixed(1)); // Fix floating point errors
        if (playerFuel < 0) playerFuel = 0;
        advanceGameTime(0.01);
    }

    // 7. Update Sector Information (Fixes Desync Bug)
    updateSectorState();

    // 8. Handle Random Encounters or Interactions
...
function handleAnomalyEncounter(anomalyObject) {
    const outcomes = [
        ...
        },
        {
            ...
            effect: () => { playerHull -= 20; if (playerHull < 1) playerHull = 1; }
        },
        {
            weight: 2, // Uncommon
...
if (playerActiveMission && playerActiveMission.type === "BOUNTY" && !playerActiveMission.isComplete) {
            ...
            playerActiveMission.objectives.forEach((obj, index) => {
                if (obj.type === "ELIMINATE") {
                    ...
                    // Update the first non-complete "ELIMINATE" objective
            ...
            if (objectiveUpdated) {
                checkMissionObjectiveCompletion(); // Check if the whole mission is done
            }
        }
...
function repairShip() {
    ...
    playerActiveMission.objectives.forEach((obj, index) => {
        ...
        if (obj.type === "ELIMINATE") {
            ...
            playerActiveMission.progress[`${progressKeyBase}_${index}`] = { acquired: 0, required: obj.count, itemID: obj.itemID, complete: false };
        }
    });
...
function handleTradeInput(key) {
    if (currentTradeContext.step === 'selectQuantity') {
        ...
        if (!isNaN(parseInt(key)) && key.length === 1 && currentQuantityInput.length < 3) {
            ...
            // No prompt update needed, handleTradeQuantity will log its own message
        } else if (key === 'l') {
            currentTradeContext = null;
            currentQuantityInput = "";
            handleInteraction();
        ...
        // Now, update the prompt *only if* we need to
        if (updatePrompt) {
            let promptMsg = getTradeQuantityPromptBaseMessage();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    saveButtonElement = document.getElementById('saveButton');
    loadButtonElement = document.getElementById('loadButton');
    const themeButtonElement = document.getElementById('themeButton'); // NEW
    ...
    saveButtonElement.addEventListener('click', saveGame);
    loadButtonElement.addEventListener('click', loadGame);
    themeButtonElement.addEventListener('click', toggleTheme); // NEW
    ...
    // 5. Apply Saved Theme Preference (NEW)
