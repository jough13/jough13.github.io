        ...
        const SWORD_PULL_MIGHT_REQ = 7; // New
        const SWORD_PULL_HP_COST = 5;   // New
        const CREATURE_DUST_COST = 1;   // New
        ...
        let devSpeedMultiplier = 1; // NEW: For developer speed controls
        ...
        const ENEMY_TYPES = {
            ...
            RUSTED_CONSTRUCT: {
                ...
                hp: 20, attack: 4, defense: 4, xp: 25,
                loot: () => {
            ...
            CRYSTAL_LURKER: {
                ...
                hp: 25, attack: 6, defense: 3, xp: 30,
                loot: () => {
        ...
        hp: 30, attack: 7, defense: 5, xp: 40,
        loot: () => {
...
const ZONE_LORE = { 
            ...
            "ASHEN_SHRINE_1": "This soot-stained shrine feels strangely warm. Carvings depict a colossal, blaonesing tree – 'The First Spark' – its branches birthing stars, its roots anchoring reality. Then, a scene of it consumed by an even greater fire – 'The Great Ember.'",
        ...
        const ARTIFACTS = [ 
            ...
            { key: "ART_ANCIENT_BLADE", name: "Ancient Blade", description: "A weapon humming with a thirsty energy. It promises power, at a cost to your vitality. (+2 MGT, -5 Max HP)" },
            { key: "ART_HEARTSTONE", name: "Heartstone", description: "A smooth, warm stone left behind by a grateful creature. It feels... peaceful. (Spirit +1)" }
        ];
        const DECISIONS = {
            "END_OF_ASHEN_WOODS": {
            ...
            "END_OF_CRIMSON_DEPTHS": { 
                ...
                options: [
                    { text: "Ascend to the Volcanic Wastes", outcomeKey: "PATH_TO_VOLCANIC_FROM_CRIMSON", nextZoneIndex: 2 },
                    { text: "Descend into the Glimmering Depths", outcomeKey: "PATH_TO_GLIMMERING_DEPTHS", nextZoneIndex: 6 }, // <-- This sends the player to our new zone!
                    { text: "End the journey here", outcomeKey: "JOURNEY_PAUSE_CRIMSON", nextZoneIndex: -1, leaveMessage: true } 
                ],
                triggeredByZoneEnd: 1 // This decision happens at the end of Zone 1
            },
...
"END_OF_SUNKEN_ARCHIVES": { 
    ...
    options: [
        { text: "Ascend to the Sky-Temple Aerie", outcomeKey: "PATH_TO_SKY_TEMPLE_FROM_ARCHIVES", nextZoneIndex: 5 },
        { text: "Delve towards the Drowned City", outcomeKey: "PATH_TO_LYRA", nextZoneIndex: 7 }, // <-- ADD THIS
...
const ZONES = [
    { // Zone 0
        ...
        foregroundElements: { 
            ...
            30: [ { lane: 2, char: 'B' }, { lane: 4, char: 'M'} ], 35: [ { lane: 0, char: 'L' }, { lane: 4, char: 'M'} ], 
            40: [ { lane: 2, char: '§' }, { lane: 4, char: 'M'} ], 
            45: [ { lane: 4, char: 'T' } ], 50: [ { lane: 1, char: 'A' } ], 55: [ { lane: 2, char: 'F'} ],
            60: [ { lane: 3, char: 'P' } ], 
            65: [ { lane: 4, char: '!' } ], 70: [ { lane: 2, char: 'E', enemyKey: "ASH_GNAWER" },{ lane: 4, char: '~' }, { lane: 0, char: 'F' } ], 
            75: [ { lane: 4, char: 'L' } ], 80: [ { lane: 0, char: 'T' } ], 
            85: [ { lane: 0, char: '~' },{ lane: 1, char: '=' },{ lane: 2, char: '~' } ], 
            90: [ { lane: 2, char: '#' } ], 95: [ { lane: 3, char: '+' } ], 100: [ { lane: 1, char: 'd' }, { lane: 4, char: 'F'} ], 
            105: [ { lane: 0, char: '0'} ], 
            ...
            160: [ { lane: 0, char: 'L' }, {lane: 4, char: '?' } ], 170: [ { lane: 2, char: 'E', enemyKey: "ASH_GNAWER" } ], 175: [ { lane: 1, char: '+' } ], 
            180: [ { lane: 3, char: 'T' } ], 190: [ { lane: 4, char: 'P' } ], 195: [ { lane: 2, char: '¥' } ], 
    ...
    { // Zone 1
        ...
        foregroundElements: {
            ...
            70: [ { lane: 0, char: 'E', enemyKey: "ASH_GNAWER" } ], 80: [ { lane: 4, char: 'N', npcType: "HERMIT_RUNESMITH" } ], 90: [ { lane: 2, char: 'Φ' } ],
            ...
            190: [ { lane: 2, char: 'N', npcType: "LOST_SOUL" } ], 200: [ { lane: 4, char: '✧' } ], 210: [ { lane: 1, char: 'E', enemyKey: "RUSTED_CONSTRUCT" } ], 
            220: [ { lane: 2, char: 'Δ' } ], 230: [ { lane: 3, char: 'H' } ], 240: [ { lane: 0, char: 'A' } ], 
            250: [ { lane: 4, char: 'L' } ], 260: [ { lane: 2, char: 'B' } ], 270: [ { lane: 1, char: 'L' } ],
            280: [ { lane: 3, char: 'P' } ]
        },
    ...
    { // Zone 3 
        ...
        foregroundElements: { 
            ...
            20: [ { lane: 0, char: 'E', enemyKey: "VOID_TENDRIL" }, { lane: 4, char: 'M'} ], 25: [ { lane: 2, char: '0'} ],
            30: [ { lane: 4, char: 'V' } ], 
            40: [ { lane: 1, char: 'A' } ], 50: [ { lane: 3, char: '+' } ], 60: [ { lane: 2, char: '.' } ], 
            70: [ { lane: 0, char: 'E', enemyKey: "VOID_TENDRIL" } ], 80: [ { lane: 4, char: '#' } ], 90: [ { lane: 1, char: 'V' } ],
            100: [ { lane: 3, char: 'N', npcType: "ECHO_LUMINA" } ], 110: [ { lane: 2, char: 'X' } ], 120: [ { lane: 0, char: 'P' } ], 
            130: [ { lane: 4, char: 'E', enemyKey: "VOID_TENDRIL" } ], 140: [ { lane: 1, char: 'A' } ], 150: [ { lane: 2, char: 'L' } ], 
            160: [ { lane: 3, char: 'X' } ], 170: [ { lane: 2, char: 'H' } ], 180: [ { lane: 0, char: '+' } ], 
            190: [ { lane: 4, char: 'E', enemyKey: "VOID_TENDRIL" } ], 200: [ { lane: 1, char: 'V' } ], 210: [ { lane: 3, char: 'L' } ], 
            220: [ { lane: 2, char: 'A' } ], 230: [ { lane: 0, char: '#' } ], 240: [ { lane: 4, char: 'E', enemyKey: "VOID_TENDRIL" } ], 
            ...
            310: [ { lane: 2, char: 'L' } ], 320: [ { lane: 3, char: 'X' } ], 330: [ { lane: 0, char: 'E', enemyKey: "VOID_TENDRIL" } ], 
    ...
    { // Zone 6 (NEW)
        name: "The Glimmering Depths", width: 300, backgroundChar: "'", midgroundChar: "✧", 
        foregroundElements: { 
            10: [ { lane: 2, char: '♦' } ], 20: [ { lane: 0, char: '◊' } ], 30: [ { lane: 4, char: '♦' } ],
            40: [ { lane: 1, char: 'A' } ], 50: [ { lane: 3, char: '#' } ], 60: [ { lane: 2, char: 'E', enemyKey: "CRYSTAL_LURKER" } ], 
            70: [ { lane: 0, char: 'N', npcType: "ECHO_LUMINA"} ], 80: [ { lane: 4, char: '♦' } ], 90: [ { lane: 1, char: 'L' } ],
            100: [ { lane: 3, char: 'P' } ], 110: [ { lane: 2, char: '◊' } ], 120: [ { lane: 0, char: 'E', enemyKey: "CRYSTAL_LURKER" } ],
            130: [ { lane: 4, char: 'A' } ], 140: [ { lane: 1, char: '♦' } ], 150: [ { lane: 3, char: 'L' } ],
            160: [ { lane: 2, char: '#' } ], 170: [ { lane: 0, char: '◊' } ], 180: [ { lane: 4, char: 'E', enemyKey: "CRYSTAL_LURKER" } ],
            190: [ { lane: 1, char: 'P' } ], 200: [ { lane: 3, char: '♦' } ], 210: [ { lane: 2, char: 'H' } ],
            220: [ { lane: 0, char: 'A' } ], 230: [ { lane: 4, char: '◊' } ], 240: [ { lane: 1, char: 'L' } ],
            250: [ { lane: 3, char: 'E', enemyKey: "CRYSTAL_LURKER" } ], 260: [ { lane: 2, char: '#' } ], 270: [ { lane: 0, char: 'P' } ],
            280: [ { lane: 4, char: '♦' } ], 290: [ { lane: 1, char: 'L' } ]
        },
        color: "#40e0d0", // Turquoise
        bgColor: "#003333", 
        entryLoreKey: "GLIMMERING_DEPTHS_INTRO", // We will add this lore key next
        shrineLoreKey: "GLIMMERING_SHRINE_1" // And this one too
    },
    { // Zone 7 (NEW)
        name: "The Drowned City of Lyra", width: 450, backgroundChar: "≈", midgroundChar: "s", 
        foregroundElements: { 
            10: [ { lane: 0, char: '~' }, { lane: 1, char: '~' }, { lane: 3, char: '~' }, { lane: 4, char: '~' } ],
            20: [ { lane: 2, char: 'S' } ], 30: [ { lane: 1, char: 'D' } ], 40: [ { lane: 3, char: '#' } ],
            50: [ { lane: 4, char: 'E', enemyKey: "RUSTED_SENTINEL" } ],
            60: [ { lane: 0, char: 'L' } ], 70: [ { lane: 2, char: 'R' } ], 80: [ { lane: 1, char: 'A' } ],
            100: [ { lane: 3, char: 'N', npcType: "ARCHIVIST_CONSTRUCT" } ],
            120: [ { lane: 0, char: 'E', enemyKey: "RUSTED_SENTINEL" } ],
            140: [ { lane: 4, char: 'D' } ], 160: [ { lane: 2, char: 'S' } ], 180: [ { lane: 1, char: 'L' } ],
            200: [ { lane: 3, char: '#' } ], 220: [ { lane: 0, char: 'R' } ],
            240: [ { lane: 4, char: 'E', enemyKey: "RUSTED_SENTINEL" } ],
            260: [ { lane: 2, char: 'H' } ], // The new Shrine
            280: [ { lane: 1, char: 'A' } ], 300: [ { lane: 3, char: 'S' } ], 320: [ { lane: 0, char: 'L' } ],
            340: [ { lane: 4, char: 'D' } ],
            360: [ { lane: 2, char: 'E', enemyKey: "RUSTED_SENTINEL" } ],
            380: [ { lane: 0, char: 'N', npcType: "ECHO_LUMINA" } ],
            400: [ { lane: 1, char: 'A' } ], 420: [ { lane: 3, char: 'R' } ],
            440: [ { lane: 2, char: 'P' } ] // Maybe a final companion find?
        },
        color: "#1d7874", // A deep teal color
        bgColor: "#0b3937", 
...
const explorationSpeedWrapper = document.getElementById('exploration-speed-wrapper');
        ...
        function calculateMaxHp() {
            ...
            return Math.max(1, hp); // Ensure HP doesn't drop below 1
        }
...
function renderStats() {
    ...
    document.getElementById('player-hp-wrapper').title = "Health Points (HP): Your vitality. If this reaches 0, your journey ends.";
    playerMaxHpDisplay.textContent = gameState.maxHp;

    playerClassDisplay.textContent = gameState.playerClass || "None";
    document.getElementById('player-class-wrapper').title = "Class: Your chosen specialization, granting unique bonuses.";

    statsExplorationSpeed.textContent = `${gameState.explorationSpeedMultiplier.toFixed(1)}x`;
    document.getElementById('exploration-speed-wrapper').title = "Exploration Speed: How quickly you travel. Can be upgraded with Glimmering Dust.";

    statsGlimmeringDust.textContent = gameState.resources.glimmeringDust;
    document.getElementById('glimmering-dust-wrapper').title = "Glimmering Dust: A common resource used for basic upgrades and actions.";

    statsAncientScraps.textContent = gameState.resources.ancientScraps;
    document.getElementById('ancient-scraps-wrapper').title = "Ancient Scraps: Remnants of a bygone era, hinting at lost technology.";

    statsRunesCollected.textContent = gameState.runes.length > 0 ? gameState.runes.join(' ') : 'None';
    document.getElementById('runes-collected-wrapper').title = "Runes: Powerful, permanent symbols you have attuned to.";
    ...
    document.getElementById('artifacts-collected-wrapper').title = "Artifacts: Unique relics from the past, each with its own story or effect.";

    statMightDisplay.textContent = effectiveStats.might;
    document.getElementById('stat-might-wrapper').title = "Might (MGT): Increases max HP, physical power in combat, and success in strength-based challenges.";
    ...
    document.getElementById('stat-wits-wrapper').title = "Wits (WIT): Increases success in deciphering runes, understanding lore, and discovering secrets.";

    statSpiritDisplay.textContent = effectiveStats.spirit;
    document.getElementById('stat-spirit-wrapper').title = "Spirit (SPR): Enhances spiritual power in combat and improves outcomes with companions, shrines, and other mystical events.";

    playerLevelDisplay.textContent = gameState.level;
    document.getElementById('player-level-wrapper').title = "Level: Your overall character level. Increases with experience (XP).";

    playerXpDisplay.textContent = gameState.xp;
    document.getElementById('player-xp-wrapper').title = "Experience (XP): Gained from discoveries and combat. Earn enough to level up.";
    ...
    upgradeSpeedButton.disabled = gameState.resources.glimmeringDust < upgradeCost || gameState.currentZoneIndex === -1;
        ...
        function lightenDarkenColor(col, amt) {
            ...
            let g = ((num >> 8) & 0x00FF) + amt; // Corrected: this is Green
            if (g > 255) g = 255; else if (g < 0) g = 0;
            let b = (num & 0x0000FF) + amt; // Corrected: this is Blue
        ...
        function resolveCombat(enemyKey) {
            ...
            function processCombatRound() {
                ...
                if (damageToPlayer > 0) {
    // You could add a "player takes damage" sound here if you have one
                } else {
                    playSound('combatMiss'); // Plays "miss" sound on a miss (0 damage)
                }       
        ...
        // NEW: This function was missing, causing stat challenges to fail.
        function presentStatChallengeDecision(flagKey, promptText, ignoreText, successCondition, failureCallback) {
            ...
            const decisionData = {
                prompt: promptText,
                options: [
                    { text: "Attempt the challenge", isChallenge: true },
                    { text: "Ignore and move on", isIgnore: true }
                ]
            };
            ...
            decisionData.options.forEach(option => {
                ...
                button.onclick = () => {
                    ...
                    if (option.isChallenge) {
                        ...
                        // Success messages are handled within the successCondition lambda now
                    } else { // Ignored
        ...
        function handleEncounter() { 
            ...
            elementsAtCurrentX.forEach(element => {
                ...
                // Skip encounters if not in the player's lane
                if (element.lane !== gameState.playerLane) {
                    return;
                }
                
                // If player moves into a blocking tile, stop. Movement logic will handle this.
                ...
                switch (currentElementChar) {
                    ...
                    case '!': 
                        ...
                        break;
                    case '¥': // NEW ENCOUNTER: Sword in the Stone
                        if (!gameState.narrativeFlags[specificEncounterKey]) {
                             presentStatChallengeDecision(
                                specificEncounterKey,
                        ...
                        break;
                    case 'd': // NEW ENCOUNTER: Dying Creature
                        if (!gameState.narrativeFlags[specificEncounterKey]) {
                            presentStatChallengeDecision(
                                specificEncounterKey,
                    ...
                    case 'F': 
                        ...
                        if (seededRandom() < ( (effectiveStats.might - BASE_STAT_VALUE) * MIGHT_BONUS_DUST_CHANCE_PER_POINT) ) {
                            dustForaged +=1;
                    ...
                    case '0': 
                        ...
                        break;
                    case 'w': 
                        if (zone.name === "The Sky-Temple Aerie") {
                            if (seededRandom() < 0.5) {
                    ...
                    case 'A': 
                        ...
                        if (!gameState.narrativeFlags[artifactKey]) {
                            ...
                            if (undiscoveredArtifacts.length > 0) {
                                ...
                                if (foundArtifact.key === "ART_TOME_MIGHT") {
                                    ...
                                    artifactXp = 20;
                                } else if (effectiveStats.wits >= WIT_INSIGHT_THRESHOLD) {
                        ...
                        break;
                    case 'N': 
                        // FIX: Was using an undefined variable for the key.
                        const npcSpecificKey = `${encounterKeyBase}_${element.lane}_${element.npcType}`;
                        ...
                        if (!gameState.narrativeFlags[loreFlagKey]) {
                            ...
                            if (undiscoveredLore.length > 0) {
                                ...
                                gameState.narrativeFlags[loreFlagKey] = true; 
                                 if (effectiveStats.wits >= WIT_INSIGHT_THRESHOLD) {
                    ...
                    case 'P': 
                        if (!gameState.companion) {
                            ...
                            awardXP(15); 
                            if (effectiveStats.spirit >= SPIRIT_COMPANION_THRESHOLD) {
                    ...
                    case '+': 
                        ...
                        if (effectiveStats.might > BASE_STAT_VALUE) { 
                            dustFromGraveChance += (effectiveStats.might - BASE_STAT_VALUE) * MIGHT_BONUS_DUST_CHANCE_PER_POINT;
                        }
                    ...
                    case '.':
                        ...
                        playSound('dust');
if (gameState.companion) {
    // If the companion is a Fox, it has a 33% chance to find a good amount of bonus dust.
    if (gameState.companion.type === "Fox" && seededRandom() < 0.33) { 
        const bonusDust = seededRandomInt(2, 4);
        gameState.resources.glimmeringDust += bonusDust;
        addLogMessage(`${gameState.companion.name}'s keen senses uncover an extra ${bonusDust} Glimmering Dust!`, "companion");
    } 
    // Any other companion has a smaller 10% chance to find just one extra dust.
    else if (seededRandom() < 0.10) { 
        gameState.resources.glimmeringDust += 1;
        addLogMessage(`${gameState.companion.name} helps you find an extra piece of dust!`, "companion");
    }
}
break;
                    case 'm': 
                    ...
                    case '#':
                        ...
                        break;
                    case '§': 
                        if (!gameState.runes.includes('§')) {
                            gameState.runes.push('§'); awardXP(20); 
                            gameState.explorationSpeedMultiplier += 0.2; updateGameTickSpeed();
                            addLogMessage("Discovered Rune of Swiftness (§)! Speed increased.", "synergy");
                            playSound('rune');
                        } else { addLogMessage("Passed an already attuned Swiftness Rune.", "lore");}
                        break;
                    case 'Φ': 
                         if (!gameState.runes.includes('Φ')) {
                            gameState.runes.push('Φ'); awardXP(20); 
                            addLogMessage("Discovered Rune of Perception (Φ)! Senses sharpened.", "synergy");
                            playSound('rune');
                        } else { addLogMessage("Passed an already attuned Perception Rune.", "lore");}
                        break;
                    case 'Δ': 
                        if (!gameState.runes.includes('Δ')) {
                            gameState.runes.push('Δ'); awardXP(20); 
                            addLogMessage("Discovered Rune of Resilience (Δ)! Fortitude increased.", "synergy");
                            playSound('rune');
                        } else { addLogMessage("Passed an already attuned Resilience Rune.", "lore");}
                        break;
                    ...
                    case '♦': case '◊': case '✧':
                        ...
                        break;
                    case '[': case ']': case '¦': case '^':
                        addLogMessage(`Passed near ancient ${getElementDescription(currentElementChar)}.`);
                        break;
                    // These are handled by movement logic, not direct encounters
                    case '~': case '=': case 'M': 
                        break;
                }
            });
        ...
        function gameLoop() { 
            ...
            if (seededRandom() < VERTICAL_MOVE_CHANCE) {
                ...
                if (newLane >= 0 && newLane < NUM_LANES) {
                    ...
                    // Check one step ahead for blocking terrain
                    const checkX = gameState.playerZoneX;
                    if (zoneForCheck && zoneForCheck.foregroundElements[checkX]) {
                        const elementsInTargetColumn = zoneForCheck.foregroundElements[checkX];
...
document.addEventListener('DOMContentLoaded', () => { 
    // Initialize sounds early
    initializeSounds(); 

    // This waits for external fonts like 'VT323' to be fully loaded
    document.fonts.ready.then(() => {
        ...
        // Setup UI and then remove the loading class to trigger the fade-in
        updateUIAccentColors(); 
        renderAll(); 
        document.body.classList.remove('loading');

        // Attach listeners and start the game loop
        pauseResumeButton.addEventListener('click', togglePause);
        upgradeSpeedButton.addEventListener('click', attemptUpgradeSpeed);
copyLogButton.addEventListener('click', () => {
    // The 'logArea' constant is already defined, so we can use it here
    const logText = logArea.innerText; // .innerText gets clean text without HTML tags
    ...
    navigator.clipboard.writeText(logText).then(() => {
        ...
        }, 1500); // Change it back after 1.5 seconds

    }).catch(err => {
        console.error('Failed to copy log: ', err);
        addLogMessage("Error: Could not copy log to clipboard.", "puzzle-fail");
    });
});
